define(
                ...
                TengwarExampleSD1, TengwarExampleSD2, TengwarExampleTLOTR2)
        {
            ...
            QUnit.test("createPhonemeToRuneMap()", function(assert)
            {
                // Setup.
                var transcriber = createTranscriber();

                // Run.
                var result = transcriber.createPhonemeToRuneMap();
                // console.info(transcriber.mapToString(result));

                // Verify.
                assert.ok(result);
                // assert.equal(Object.keys(result).length, 154);
                assert.equal(result[" "], "space");
                assert.equal(result["the"], "the");

                {
                    var value = result[[ "a", "j", "e" ]];
                    // LOGGER.debug("value = " + value);
                    assert.equal(value.length, 3);
                    assert.equal(value[0], "threeDots");
                    assert.equal(value[1], "anga");
                    assert.equal(value[2], "underDot");
                }
                /*
                 * { var value = result[[ "s", [ "o", "n" ] ]]; // LOGGER.debug("value = " + value);
                 * assert.equal(value.length, 2); assert.equal(value[0], "silme"); assert.equal(value[1][0],
                 * "leftCurl"); assert.equal(value[1][1], "numen"); }
                 */
            });

            QUnit.test("createWordToCountMap()", function(assert)
            {
                // Setup.
                var transcriber = createTranscriber();

                // Run.
                var result = transcriber.createWordToCountMap();

                var properties = Object.getOwnPropertyNames(result).sort();
                var number = 0;
                var string = "";
                ...
                for (var i = 0; i < properties.length; i++)
                {
                    ...
                    var count = result[property];
                    if (count > 1)
                    {
                        number++;
                        string += number + " " + property + ": " + count + "\n";
                    }
                }
            ...
            QUnit.test("createWordToRuneWordMap()", function(assert)
            {
                // Setup.
                var transcriber = createTranscriber();

                // Run.
                var result = transcriber.createWordToRuneWordMap();
                ...
                for (var i = 0; i < properties.length; i++)
                {
                    ...
                    if (runeWord.length > 1)
                    {
                        ...
                        string += "\n";
                        for (var j = 0; j < runeWord.length; j++)
                        {
                            string += transcriber.arrayToString(runeWord[j]);
                            string += "\n";
                        }
                    }
                }
